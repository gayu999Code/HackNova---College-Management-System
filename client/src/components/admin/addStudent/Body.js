...
const Body = () => {
  ...
  // Helper function to stringify error objects
  const getErrorMessage = (errorObj) => {
    if (typeof errorObj === 'string') return errorObj;
    if (errorObj && typeof errorObj === 'object') {
      // Handle MongoDB duplicate key error or other complex error objects
      if (errorObj.keyValue) {
        return `Duplicate entry: ${Object.keys(errorObj.keyValue)[0]} "${Object.values(errorObj.keyValue)[0]}" already exists.`;
      }
      // Return a stringified representation or a default message
        ...
        <div className=" mr-10 bg-white flex flex-col rounded-xl ">
          <form
            ...
            <div ref={errorRef} className={classes.loadingAndError}>
              ...
              {(error.emailError || error.backendError) && (
                <p className="text-red-500">
                  {typeof error.emailError === 'string' 
                    ? error.emailError 
                    : typeof error.backendError === 'string'
                      ? error.backendError
                      : getErrorMessage(error.emailError || error.backendError)}
                </p>
              )}
            </div>
          </form>
